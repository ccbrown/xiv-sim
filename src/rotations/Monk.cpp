...
const Action* Monk::nextAction(const Actor* subject, const Actor* target) const {
	...
	if (subject->globalCooldownRemaining() > 1_s && subject->auraCount("greased-lightning") == 3 && subject->auraCount("twin-snakes")) {
	...
	if (target->auraTimeRemaining("demolish-dot") < 4_s && IsCoeurlForm(subject)) {
		return models::Monk::DemolishRear;
	}

	if ((subject->auraTimeRemaining("greased-lightning") < 4_s || subject->auraCount("greased-lightning") < 3) && IsCoeurlForm(subject)) {
		return target->auraTimeRemaining("demolish-dot") < 4_s ? models::Monk::DemolishRear : models::Monk::SnapPunchFlank;
	}

	if (IsRaptorForm(subject) && subject->auraTimeRemaining("twin-snakes") < 4_s) {
		return models::Monk::TwinSnakesFlank;
	}

	if (IsOpoOpoForm(subject) && target->auraTimeRemaining("dragon-kick") < 4_s && !subject->auraCount("perfect-balance")) {
